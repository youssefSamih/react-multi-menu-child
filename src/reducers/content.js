...
const findItem = (content, value, prop) =>
  content.find((i) => i[prop] === value);
...
const content = (state = [], action) => {
  switch (action.type) {
    ...
    case "CHANGE_ACTIVE_LINK": {
      ...
      const stage = state.map((i) => item(i, { id, trace, ...action }));

      // Trace also keeps parentId nonetheless it doesn't matter
      return content(
        stage,
        changeSubMenuVisibility(action.reduxUid, parentId, trace, true)
      );
    }
    default: {
      return state;
